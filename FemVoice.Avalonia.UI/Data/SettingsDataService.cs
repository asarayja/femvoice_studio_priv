    ...
    public DataOpResult Backup(DateTime nowLocal)
    {
        try
        {
            if (!File.Exists(_dbPath))
                return new DataOpResult(false, "Ingen database å sikkerhetskopiere ennå.");
            ...
            return new DataOpResult(true, $"Sikkerhetskopi lagret: {name}");
        }
        catch (Exception ex) { return new DataOpResult(false, $"Sikkerhetskopi feilet: {ex.Message}"); }
    }
    ...
    public DataOpResult Restore(string backupPath, DateTime nowLocal)
    {
        try
        {
            if (string.IsNullOrWhiteSpace(backupPath) || !File.Exists(backupPath))
                return new DataOpResult(false, "Fant ikke sikkerhetskopien.");
            Directory.CreateDirectory(_backupDir);
            ...
            return new DataOpResult(true, "Gjenopprettet. Start appen på nytt for at endringen skal gjelde fullt ut.");
        }
        catch (Exception ex) { return new DataOpResult(false, $"Gjenoppretting feilet: {ex.Message}"); }
    }
    ...
    public DataOpResult Clear()
    {
        try
        {
            if (_database is FemVoiceStudio.Data.DatabaseService concrete)
            {
                ...
                return new DataOpResult(true, "Databasen er tømt (tom, klar til bruk).");
            }
            if (File.Exists(_dbPath))
            {
                File.Delete(_dbPath);
                return new DataOpResult(true, "Databasen er tømt. Start appen på nytt for å opprette en ny tom database.");
            }
            return new DataOpResult(false, "Ingen database å tømme.");
        }
        catch (Exception ex) { return new DataOpResult(false, $"Tømming feilet: {ex.Message}"); }
    }
}
