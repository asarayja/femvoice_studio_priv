...
| `FemVoiceStudio.Tests/ResonanceContrastDemoTests.cs` | **(CI run 27617978406)** 4 failures: the test reads the RESX via `RepoRoot()/"FemVoiceStudio"/"Resources"`, but the RESX moved to `FemVoice.Core/Resources` → `DirectoryNotFoundException`. Repointed `ResourcesDir()` to `"FemVoice.Core","Resources"` (the test's `Views/` paths are unchanged — Views stayed in WPF). Port-induced path fix; same class as the portable-test path fixes. No assertion change. | No | Windows CI re-run | Low |

> **Pre-existing (NOT fixed — git-proven):** `ThemeNoteButtonStyleTests` (4 failures: `NoteRadioButtonStyle_ExistsAndCoversAllStates`/`UsesThemeBrushes` for Dark/Light) reads `FemVoiceStudio/Themes/*.xaml` (which did **not** move) + `AnalyzerWindow.xaml.cs` (also unchanged). Both inputs are **byte-identical to `origin/main`**, so the result equals the pre-port baseline — a pre-existing theme-style assertion failure, not caused by the migration. Frozen theme XAML and the assertion are left unchanged.
