...
| ReportExportWindow — 4 report types × 3 formats (QuestPDF/CSV) | 🟡 Reports | **REAL** progress-summary preview + **real CSV/text export** of saved sessions via a native save dialog (PR #57, #64). Full 4×3 clinical generation (OutcomeProfile/notes/audit assembler) + PDF/QuestPDF deferred. |
...
| ClinicianDashboard · CoachDashboard · CaseReviewWindow | 🟡 Veilederpanel (Coach) | **REAL** coach panel (PR #66): assembles a real `OutcomeProfile` → `CoachReport` from saved sessions via the frozen Core pipeline (SmartCoach + effectiveness + longitudinal engines, read-only) and shows focus areas / recommendations / detected development. Opened from the Reports page. Clinician + CaseReview panels still deferred. Degrades to a truthful "not enough data" state; per-dimension resonance/intonation fill in once sessions record them. |
