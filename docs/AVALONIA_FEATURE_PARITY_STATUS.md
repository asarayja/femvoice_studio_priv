...
| AnalysisWindow — OxyPlot resonance/pitch/prosody/health over real session data | 🟡 Analysis | **REAL** pitch/score/**resonance** trends + summary stats from the DB (PR #56, #70). Resonance now flows end-to-end (dashboard DSP → `ResonanceScore` persisted via `UpdateTrainingSession` → Analysis trend). OxyPlot styling + intonation trend deferred. |
