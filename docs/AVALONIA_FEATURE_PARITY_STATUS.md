...
| SmartCoachDashboard/Detail — engine-backed daily recommendation, streak, health | ✅ SmartCoach | **REAL** engine: `SmartCoachEngine.GenerateDailyRecommendation` on the real DB (PR #53) PLUS the **Detail** metrics (day streak, sessions/total-time this week, consistency, 7-day weekly history) from real DB stats (PR #78) — Dashboard + Detail merged into one screen. |
...
| AnalyzerWindow | ✅ Analysator | **REAL** real-time analyzer (PR #79): live main frequency (Core pitch) + resonance (Core engine) vs a selectable target frequency, plus running pitch statistics (avg/min/max/duration/voiced-sample count). Nav item; measurement-only. |
| StatisticsWindow | 🟡 Statistikk | **REAL** stats (total/streak/days/time/avg pitch/consistency/score) from the DB (PR #58). |
| CalendarWindow · DayDetailsWindow | ✅ Kalender + Øktdetaljer | **REAL** training-day history (last 90 days) with **clickable days** → a **DayDetails** panel showing every session that day (time/duration/pitch/resonance/score) from the DB (PR #59, #76). |
| ClinicianDashboard · CoachDashboard · CaseReviewWindow | ✅ Veileder + Kliniker + Saksgjennomgang | **REAL** Coach (#66), **Clinician** (#67), and **CaseReview** (#77) panels — all assemble a real `OutcomeProfile` from saved sessions via the frozen Core pipeline (read-only). Coach = focus/recommendations/development; Clinician = OutcomeReport overview; CaseReview = review-type + period-scoped outcome summary **with saved-review persistence** (PR #81 — CaseReviewAssembler → CaseReviewsStore.SaveAsync, saved reviews listed). All opened from the Reports page; all degrade to a truthful "not enough data" state. |
| ManualOverrideWindow (safety clamp) | ✅ Manuell overstyring | **REAL safety clamp** (PR #74) + **audit-log persistence** (PR #80): runs the FROZEN `ManualOverrideEngine` clamp verbatim, shows ONLY the clamped outcome, and now logs the clamped RESULT to the `ManualOverridesStore` (SQLite) via `LogResultAsync` — the log stores outcome flags + metadata, NEVER the raw profile, so the safety invariant carries through to persistence. Recent overrides listed. No safety logic changed. |
