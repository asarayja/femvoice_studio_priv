...
| 8f | Desktop shell + navigation/layout parity | **SLICE DONE (Linux-verified, PR open)** — window chrome (min-size + CenterScreen + resizable); header + left nav rail (`ShellNavItem`: 2 implemented + 7 **deferred** static placeholders via `DeferredSurfaceViewModel`) + central content + static right info sidebar + **display-only** bottom status strip; preserved transient-page disposal (no orphaned capture/duplicate runtime); `--shell-smoke`. Converter-free; no clinical/WPF change | Agent 8 | per-destination real surfaces (Settings/Analysis/Reports/…) deferred; theme/loc adapter + packaging separate slices |
